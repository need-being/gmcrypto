...
// Size of a SM3 checksum in bytes.
const Size = 32

// BlockSize of SM3 in bytes.
const BlockSize = 64
...
func (d *digest) UnmarshalBinary(b []byte) error {
	if len(b) < len(magic) || string(b[:len(magic)]) != magic {
		return errors.New("gmcrypto/sm3: invalid hash state identifier")
	}
	if len(b) != marshaledSize {
		return errors.New("gmcrypto/sm3: invalid hash state size")
	}
	b = b[len(magic):]
